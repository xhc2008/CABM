...
async def tts_post_endpoint(request: TTS_Request_role):
    ...
    with open(os.path.join(filepath ,"role",req["role"],"config.json"), "rb") as f:
        config = json.load(f)
    data = {
        "text": req["text"],                   # str.(required) text to be synthesized
        "text_lang": "zh",              # str.(required) language of the text to be synthesized
        "ref_audio_path": os.path.join(filepath ,"role",req["role"],config["ref_audio"]),         # str.(required) reference audio path
        "aux_ref_audio_paths": [],    # list.(optional) auxiliary reference audio paths for multi-speaker tone fusion
        "prompt_text": config["ref_text"],            # str.(optional) prompt text for the reference audio
