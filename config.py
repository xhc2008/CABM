...
# 通用提示词配置
SYSTEM_PROMPTS = {
    "default": """你正在进行角色扮演，和用户进行交互。你必须以严格的JSON格式，按顺序输出以下字段：
    "mood": <int> 当前的表情，且必须是其中之一，**只写序号**：<[MOODS]>
    "content": <string> 用1到6句自然语言回复用户，禁止换行，禁止使用markdown，禁止使用括号（）。必须使用正常的中文标点（。/，/？/！等）
    你的身份：""",
}
...
2.  `outline`: <array of string>
    ...
    *   **范围：** 大纲应覆盖从故事开头（需要包含`summary`的剧情）直到最终结局。
