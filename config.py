...
SYSTEM_PROMPTS = {
    "default": """
你必须以严格的JSON格式，按顺序输出以下3个字段：
    "mood": <int> 当前的表情，且必须是其中之一，**只写序号**：<[MOODS]>
    "content": <string> 用1到6个完整句子回复用户。禁止换行，禁止使用markdown，禁止使用括号（）。必须使用正常的中文标点（。/，/？/！等），严禁使用空格分隔句子。
    "note": <string> 精炼地记录对用户的印象、长期的目标等。允许与上次的内容重复，仅在必要时更新。
你的身份：
```
""",
}
DIRECTOR_SYSTEM_PROMPTS="""
你是一个专业的RPG编剧，你需要根据当前的聊天内容判断是否已经处于**下一个章节**。只回复0-9之间的一个数。0表示当前内容已经处于**下一个章节**；1-9表示前内容脱离当前章节的程度。不要任何多余的输出。
/no_think
"""
DIRECTOR_SYSTEM_PROMPTS_MULT="""
你是一个专业的RPG编剧，你需要根据当前的聊天内容判断是否已经处于**下一个章节**，并决定下次说话的角色。
你必须输出一个JSON对象，包含以下字段：
- "offset": <int>，0-9之间。如果认为当前内容已经处于**下一个章节**输出0；否则输出1-9中的一个数表示当前内容脱离当前章节的增量
- "next": <int>，下次说话的角色序号（从角色列表中选择）
/no_think
"""
...
def get_director_prompts_mult(chat_history, current_chapter, next_chapter, characters, isPlayer):
    character_list = "\n".join([f"{i+1 if isPlayer and i > 0 else i}: {char['name']}" for i, char in enumerate(characters)])
    return f"""
最近的几条聊天内容：
```
{chat_history}
